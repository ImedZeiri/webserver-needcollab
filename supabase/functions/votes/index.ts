    ...
    switch (method) {
      case 'GET':
        const { data: getResult, error: getError } = id
          ? await supabase.from('votes').select('*').eq('id', id).single()
      ...
      case 'POST':
        ...
        const { data: postResult, error: postError } = await supabase.from('votes').insert(postBody).select();
      ...
      case 'PUT':
        ...
        const { data: putResult, error: putError } = await supabase.from('votes').update(putBody).eq('id', id).select();
        if (putError) throw putError;
        return new Response(JSON.stringify(putResult), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });

      case 'DELETE':
        const { error: deleteError } = await supabase.from('votes').delete().eq('id', id);
