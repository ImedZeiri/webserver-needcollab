  ...
  try {
    switch (method) {
      ...
      case 'POST':
        ...
        const { data: postResult, error: postError } = await supabase.from('messages').insert(postBody).select();
      ...
      case 'PUT':
        ...
        const { data: putResult, error: putError } = await supabase.from('messages').update(putBody).eq('id', id).select();
